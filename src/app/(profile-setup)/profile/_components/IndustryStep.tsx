...
const choices: Array<[string, LucideIcon]> = [["Real Estate", Building2], ["Technology", BriefcaseBusiness], ["Healthcare", HeartPulse], ["Finance", Landmark], ["Legal", Scale], ["Retail", ShoppingBag], ["Construction", Wrench], ["Consulting", BriefcaseBusiness], ["Education", GraduationCap], ["Other", Sparkles]];
export function IndustryStep({ value, onChange, onBack, onNext }: { value: string; onChange: (value: string) => void; onBack: () => void; onNext: () => void }) {
  return <StepLayout title="What's your industry?" description="We'll train your AI agents with industry-specific knowledge.">
    <div className="grid gap-2 sm:grid-cols-2" role="radiogroup" aria-label="Industry">{choices.map(([label, Icon]) => <button type="button" role="radio" aria-checked={value === label} onClick={() => onChange(label)} className={cn("flex h-11 items-center gap-3 rounded-md border px-4 text-left text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5d7df3]", value === label ? "border-[#6f8df3] bg-[#dfe7ff] text-[#4c70ea]" : "border-transparent bg-[#f4f6fd] text-[#222738] hover:border-[#cbd5f8]")} key={label}><Icon className="size-4" />{label}</button>)}</div>
