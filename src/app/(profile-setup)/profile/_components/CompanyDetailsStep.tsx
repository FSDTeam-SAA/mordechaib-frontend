...
const fields = [
  ...
  ["phone", "Phone Number", "+012 3456 7891", "tel", true], ["businessHours", "Business hours", "9:00 AM to 5:00 PM", "text", true],
] as const;
const addressFields = [["city", "City", "Washington DC"], ["street", "Street", "82/1 road"], ["state", "State", "Washington DC"], ["zipCode", "State zip code", "1234"]] as const;
...
export function CompanyDetailsStep({ data, onChange, onNext }: Props) {
  ...
  const renderInput = (key: keyof ProfileFormData, label: string, placeholder: string, type = "text", required = true) => (
    <label className="space-y-1.5 text-base font-normal text-[#8B93B8]" key={key}>
      <span>{label}</span>
      <Input type={type} value={data[key]} onChange={(event) => onChange({ [key]: event.target.value })} placeholder={placeholder} required={required} className="h-10 border-0 bg-[#f4f6fd] text-sm shadow-none placeholder:text-[#a4aecb] focus-visible:ring-[#5d7df3]" />
    </label>
  );
  return (
    <StepLayout title="Set Up Your Business Profile" description="Your AI agents will represent this business.">
      <form onSubmit={submit}>
        ...
        <Button type="submit" className="mt-5 h-11 w-full bg-[#5b7ced] hover:bg-[#496be0]">Next</Button>
      </form>
    </StepLayout>
  );
}
