...
export function CompleteStep({ onBack, onComplete }: { onBack: () => void; onComplete: () => void }) {
  ...
  return <StepLayout title="You're All Set" description="Your AI workforce is ready. Let's start with your first voice note." showLogo={false} headerIcon={successIcon}>
    <div className="flex gap-2"><Button type="button" variant="outline" onClick={onBack} className="h-11 w-24 border-[#1c2538]">Back</Button><Button type="button" onClick={onComplete} className="h-11 flex-1 bg-[#5b7ced] hover:bg-[#496be0]">Continue To Dashboard</Button></div>
  </StepLayout>;
}
